...
code_challenge_method = Regex.run(~r/PKCE_CODE_CHALLENGE_METHOD = "([^"]+)"/, pkce_content) |> List.last()
...
case TheMaestro.Auth.exchange_code_for_tokens(auth_code, pkce_params) do
  {:ok, oauth_token} ->
    ...
    IO.puts("Expires At: #{if oauth_token.expiry, do: DateTime.from_unix!(oauth_token.expiry), else: "N/A"}")
    IO.puts("Refresh Token: #{if oauth_token.refresh_token, do: String.slice(oauth_token.refresh_token, 0, 30) <> "...", else: "N/A"}")
