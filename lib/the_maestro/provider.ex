defmodule TheMaestro.Provider do
  ...
  @spec delete_session(Types.provider(), Types.auth_type(), session_id()) :: :ok | {:error, term()}
  ...
  @spec list_models(Types.provider(), Types.auth_type(), session_id()) :: {:ok, [Types.model_id()]} | {:error, term()}
  ...
  @spec resolve_module(Types.provider(), operation()) :: {:ok, module()} | {:error, :module_not_found}
  ...
  def build_module_path(provider, operation) do
    Module.concat([TheMaestro, :Providers, provider_to_module(provider), operation_to_module(operation)])
  end

  @spec validate_provider_compliance(module()) :: :ok | {:error, [String.t()]}
  def validate_provider_compliance(mod) when is_atom(mod) do
    errors = []
    errors = if function_exported?(mod, :create_session, 1), do: errors, else: ["create_session/1 not implemented" | errors]
    errors = if function_exported?(mod, :delete_session, 1), do: errors, else: ["delete_session/1 not implemented" | errors]
    errors = if function_exported?(mod, :refresh_tokens, 1), do: errors, else: ["refresh_tokens/1 not implemented" | errors]
    errors = if function_exported?(mod, :stream_chat, 3), do: errors, else: ["stream_chat/3 not implemented" | errors]
    errors = if function_exported?(mod, :list_models, 1), do: errors, else: ["list_models/1 not implemented" | errors]
