defmodule TheMaestro.Providers.Gemini.APIKey do
  ...
  def create_session(opts) when is_list(opts) do
    ...
    APIKeyHelper.create_session(:gemini, opts, &validate_api_key/1)
  end
  ...
  def delete_session(session_id) when is_binary(session_id) do
    ...
    APIKeyHelper.delete_session(:gemini, :api_key, session_id)
  end
  ...
  def test_connection(%Req.Request{} = req) do
    # CRITICAL: Keep Gemini-specific endpoint exactly as is
    # Only the response handling is shared
    ConnectionTester.test_connection(req, "/v1beta/models")
  end
end
