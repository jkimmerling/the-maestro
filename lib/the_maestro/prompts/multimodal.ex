defmodule TheMaestro.Prompts.MultiModal do
  ...
  def process_multimodal_content(content, context) do
    ...
    with {:ok, validated_content} <- validate_content_structure(content),
      ...
      %{
        ...
        performance_metrics: Map.put(optimized_result.performance_metrics, :total_processing_time_ms, processing_time),
  ...
  defp validate_single_content_item(%{type: type, content: content} = item) when not is_nil(content) do
  ...
  defp enhance_accessibility(processed_content, context) do
    ...
    if Enum.empty?(accessibility_requirements) do
      ...
      AccessibilityEnhancer.enhance_content_accessibility(processed_content, accessibility_requirements)
    end
  end
  ...
  defp assess_provider_compatibility(processed_content, context) do
    ...
    content_for_assessment = Enum.map(processed_content, fn item ->
      %{
        type: item.type,
  ...
  defp apply_performance_optimizations(processed_content, context) do
    ...
    optimization_result = PerformanceOptimizer.optimize_processing_pipeline(processed_content, performance_constraints)
  ...
  defp assemble_final_prompt(optimized_result, _context) do
    content_summaries = Enum.map(optimized_result.optimized_content, &summarize_content_for_prompt/1)
  ...
  defp summarize_content_for_prompt(%{type: type, content: content} = item) do
    ...
    case type do
      ...
      :audio ->
        transcript = get_in(processed, [:transcription, :text]) || "Audio content (transcript not available)"
  ...
  defp calculate_item_complexity(%{type: type, metadata: metadata}) do
    ...
    size_factor = case Map.get(metadata, :size_mb) do
      nil -> 1.0
    ...
    optimized_content = Enum.map(content, fn item ->
      case item do
        ...
        %{type: :video} ->
          ...
          %{item | type: :image, metadata: Map.put(item.metadata || %{}, :converted_from, :video)}
        
        %{type: :audio} ->
          # Convert audio to transcript for Anthropic
          %{item | type: :text, metadata: Map.put(item.metadata || %{}, :converted_from, :audio)}
    ...
    optimized_content = Enum.map(content, fn item ->
      case item do
        %{type: :audio} ->
          ...
          new_metadata = Map.put(item.metadata || %{}, :preprocessing_required, :whisper_transcription)
          %{item | metadata: new_metadata}
