defmodule TheMaestro.Prompts.Optimization.Config.OptimizationConfig do
  ...
  def get_all_provider_configs do
    base_configs = Application.get_env(:the_maestro, :prompt_optimization, [])
    |> Enum.into(%{})
  ...
  def update_provider_config(provider, config_updates) when is_atom(provider) and is_map(config_updates) do
  ...
  defp get_required_config_keys(:anthropic) do
    [:max_context_utilization, :reasoning_enhancement, :structured_thinking, :safety_optimization, :context_navigation]
  end

  defp get_required_config_keys(:google) do
    [:multimodal_optimization, :function_calling_enhancement, :large_context_utilization, :integration_optimization, :visual_reasoning]
  end

  defp get_required_config_keys(:openai) do
    [:consistency_optimization, :structured_output_enhancement, :token_efficiency_priority, :reliability_optimization, :format_specification]
  end
  ...
  defp validate_config_values(:google, config) do
    ...
    valid_booleans = 
      [:multimodal_optimization, :function_calling_enhancement, :integration_optimization, :visual_reasoning]
      |> Enum.all?(fn key ->
        case Map.get(config, key) do
          value when is_boolean(value) -> true
          _ -> false
  ...
  defp validate_config_values(:openai, config) do
    ...
    valid_booleans = 
      [:consistency_optimization, :structured_output_enhancement, :reliability_optimization, :format_specification]
      |> Enum.all?(fn key ->
        case Map.get(config, key) do
          value when is_boolean(value) -> true
          _ -> false
