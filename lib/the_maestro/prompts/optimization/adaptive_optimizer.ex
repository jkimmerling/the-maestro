defmodule TheMaestro.Prompts.Optimization.AdaptiveOptimizer do
  ...
  defp identify_effective_styles(history) do
    ...
    style_performance = Enum.group_by(history, & &1[:instruction_style])
    |> Enum.map(fn {style, interactions} ->
      success_rates = Enum.map(interactions, & &1[:success_rate]) 
                     |> Enum.filter(& &1 != nil)
      avg_success = case success_rates do
        [] -> 0.0
  ...
  defp calculate_optimal_lengths(history) do
    ...
    length_quality = Enum.group_by(history, & &1[:context_length])
    |> Enum.map(fn {length, interactions} ->
      quality_scores = Enum.map(interactions, & &1[:response_quality]) 
                      |> Enum.filter(& &1 != nil)
      avg_quality = case quality_scores do
        [] -> 0.0
  ...
  defp classify_effective_examples(history) do
    ...
    |> Enum.map(fn {example_type, interactions} ->
      avg_success = Enum.map(interactions, & &1[:success_rate]) 
                   |> Enum.sum() 
  ...
  defp extract_reasoning_patterns(history) do
    ...
    |> Enum.map(fn {pattern, interactions} ->
      avg_quality = Enum.map(interactions, & &1[:reasoning_quality] || &1[:success_rate] || 0.5) 
                   |> Enum.sum() 
                   |> Kernel./(length(interactions))
      {pattern, avg_quality}
    end)
  ...
  defp identify_validation_issues(strategy) do
    ...
    issues = if strategy.preferred_instruction_style == nil do
      ["no_preferred_instruction_style" | issues]
    else
      issues
    end
    
    issues = if strategy.optimal_context_length == 0 do
      ["invalid_context_length" | issues]
    else
      issues
    end
    
    issues = if strategy.optimal_context_length > 100_000 do
      ["context_length_too_high" | issues]
    else
      issues
    end
    
    issues = if Enum.empty?(strategy.error_prevention_strategies) do
    ...
    issues = if strategy.preferred_instruction_style in [:unclear, :confusing] do
