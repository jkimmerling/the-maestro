defmodule TheMaestro.Prompts.MultiModal.Providers.ProviderCompatibilityAssessor do
  ...
  def optimize_for_provider(content, provider) do
    case provider do
      :anthropic ->
        optimize_for_anthropic(content)

      :google ->
        optimize_for_google(content)

      :openai ->
        optimize_for_openai(content)
  ...
  defp generate_image_compatibility_issues(size_mb, format, capabilities) do
    ...
    issues =
      if size_mb > Map.get(capabilities, :max_image_size_mb, 0) do
    ...
    issues =
      if format not in Map.get(capabilities, :supported_image_formats, []) do
    ...
    issues =
      if not capabilities.supports_images do
  ...
  defp generate_audio_alternatives(capabilities) do
    ...
    alternatives =
      if Map.get(capabilities, :whisper_integration_available, false) do
  ...
  defp suggest_item_adaptations(%{type: type} = item, capabilities) do
    ...
    case type do
      :image when adaptations_needed ->
        ...
        suggested_changes =
          if size_mb > max_size,
            do: [:reduce_file_size, :compress_image | suggested_changes],
            else: suggested_changes

        suggested_changes =
          if format not in Map.get(capabilities, :supported_image_formats, []),
            do: [:convert_format | suggested_changes],
            else: suggested_changes
    ...
    case type do
      ...
      :video ->
        %{
          ...
          # Significant loss converting to frames
      ...
      _ ->
        ...
        if length(accessibility_requirements) > 0 do
          %{
            ...
            # Improvement due to accessibility
  ...
  defp create_fallback_representations(content) do
    ...
    video_items =
      Enum.filter(content, &(&1.type == :video && Map.get(&1.metadata, :has_slides, false)))

    slide_images =
      Enum.flat_map(video_items, fn _video ->
        Enum.map(1..5, fn i ->
    ...
    audio_items =
      Enum.filter(content, &(&1.type == :video && Map.get(&1.metadata, :has_audio, false)))
  ...
  defp analyze_content_requirements(content) do
    ...
    has_large_files =
      Enum.any?(content, fn item -> Map.get(item.metadata || %{}, :size_mb, 0) > 50 end)
  ...
  defp get_required_adaptations(provider, content_analysis) do
    ...
    adaptations =
      if :video in content_analysis.content_types and
           not Map.get(capabilities, :supports_video, false) do
    ...
    adaptations =
      if :audio in content_analysis.content_types and
           not Map.get(capabilities, :supports_audio, false) do
  ...
  defp generate_cost_analysis(content, context) do
    ...
    cost_by_provider = %{
      # Sample pricing
      anthropic: estimated_tokens * 0.003,
      ...
      :economy ->
        %{
          optimization_suggestions: [
            ...
            "Use text alternatives where possible"
          ]
        }
    end
  end
  ...
  defp extract_specialized_features(recommendation) do
    ...
    %{
      document_analysis_capability:
        Map.get(capabilities, :supports_documents, false) |> bool_to_score(),
      multimodal_reasoning:
        score_multimodal_reasoning(Map.get(capabilities, :multimodal_reasoning, :none)),
  ...
  defp performance_monitoring_loop(state) do
    receive do
      {:record_session, session_data} ->
        ...
        current_data =
          Map.get(state.performance_data, provider, %{
            sessions: [],
            total_time: 0,
            total_quality: 0.0
          })
  ...
  defp generate_performance_report(performance_data) do
    ...
    fastest_provider =
      report
      |> Enum.min_by(fn {_provider, data} -> data.average_response_time_ms end, fn ->
        {:none, %{}}
      end)
      |> elem(0)
