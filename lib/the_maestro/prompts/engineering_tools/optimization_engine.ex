defmodule TheMaestro.Prompts.EngineeringTools.OptimizationEngine do
  ...
  defp generate_efficiency_suggestions(prompt) do
    ...
    suggestions =
      if Regex.match?(~r/\b(please|could|would)\b/i, prompt) do
    ...
    suggestions =
      if Regex.match?(~r/\b(very|really|quite|somewhat)\b/i, prompt) do
  ...
  defp recommend_optimizations(prompt) do
    ...
    recommendations =
      if String.length(prompt) > 300 do
        [
          %{type: :length, suggestion: "Consider shortening for better focus", priority: :medium}
          | recommendations
        ]
      else
        recommendations
    ...
    recommendations =
      if not Regex.match?(~r/\?/, prompt) do
        [
          %{
            type: :clarity,
            suggestion: "Add specific questions to guide response",
            priority: :high
          }
          | recommendations
        ]
  ...
  defp calculate_readability(prompt) do
    ...
    complex_words =
      Enum.count(words, fn word ->
        ...
        # Approximate for complex words
        String.length(clean_word) > 6
      end)
    ...
    sentence_complexity =
      case avg_words_per_sentence do
        # Very complex
        x when x > 25 -> 0.2
        # Complex
        x when x > 20 -> 0.4
    ...
    readability_score =
      sentence_complexity * 0.4 + vocabulary_simplicity * 0.4 - technical_penalty + 0.2
  ...
  defp calculate_specificity(prompt) do
    ...
    if word_count == 0 do
      ...
      specific_patterns = [
        ...
        {~r/\b(analyze|create|write|generate|calculate|extract|transform|summarize|classify)\b/i,
         0.08},
  ...
  defp calculate_structure_quality(prompt) do
    ...
    completeness_score =
      case {has_intro, has_conclusion} do
        {true, true} -> 0.3
        {true, false} -> 0.15
        {false, true} -> 0.15
        {false, false} -> 0.0
    ...
    topic_shifts =
      ...
      |> Enum.count(fn [s1, s2] ->
        ...
        if union_size > 0 do
          ...
          # Less than 10% word overlap suggests topic shift
          similarity < 0.1
        else
          false
        end
  ...
  defp calculate_context_clarity(prompt) do
    ...
    ambiguous_count =
      Enum.count(String.split(prompt), fn word ->
        clean_word = String.replace(word, ~r/[^a-zA-Z]/, "") |> String.downcase()
  ...
  defp calculate_instruction_clarity(prompt) do
    ...
    instruction_markers = [
      ...
      # Clear prohibitions
      {~r/\b(do not|don't|avoid|never)\b/i, 0.08}
    ]
    ...
    clarity_score =
      instruction_score + imperative_score + format_clarity - modal_penalty - complexity_penalty
  ...
  defp assess_context_relevance(prompt, context) do
    ...
    if is_nil(context) or context == "" do
      ...
      if MapSet.size(prompt_words) == 0 do
        ...
        technical_boost =
          if String.match?(context, ~r/\b(api|json|sql|algorithm|data|system)\b/i) do
            0.1
          else
            0.0
          end
  ...
  defp assess_context_completeness(prompt, context) do
    ...
    if is_nil(context) or context == "" do
      ...
      if total_requirements == 0 do
        # Default score if no specific requirements detected
        0.7
      else
