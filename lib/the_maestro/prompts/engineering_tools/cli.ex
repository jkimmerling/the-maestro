defmodule TheMaestro.Prompts.EngineeringTools.CLI do
  ...
  def show_help(parsed_command \\ %{}, _context \\ %{}) do
    help_text =
      case parsed_command[:resource] do
      ...
      String.starts_with?(arg, "--") ->
        case String.split(String.slice(arg, 2..-1//1), "=", parts: 2, trim: true) do
          ...
          [key, value] ->
            parse_args_helper(
              rest,
              {current_name, Map.put(opts, String.to_atom(key), parse_value(value)), nil}
            )
        end
  ...
  defp list_prompts(parsed_command, _context) do
    ...
    prompts =
      case load_prompts_from_storage() do
  ...
  defp list_templates(parsed_command, _context) do
    ...
    formatted_output =
      Enum.map(filtered_templates, fn template ->
  ...
  defp experiment_results(_parsed_command, _context),
    do: {:ok, "Experiment results functionality"}
  ...
  defp analyze_performance_cli(_parsed_command, _context),
    do: {:ok, "Performance analysis functionality"}
