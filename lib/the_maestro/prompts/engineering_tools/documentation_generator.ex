defmodule TheMaestro.Prompts.EngineeringTools.DocumentationGenerator do
  ...
  def generate_template_api_docs(templates, options \\ %{}) do
    ...
    try do
      api_docs = %{
        ...
        description:
          options[:description] || "Auto-generated API documentation for prompt templates",
      ...
      templates when is_list(templates) ->
        Enum.flat_map(templates, fn template ->
          generate_template_examples(
            template,
            div(example_count, length(templates)),
            complexity_levels
          )
        end)
  ...
  defp extract_prompt_title(prompt) do
    ...
    if String.length(first_line) < 100 and
         String.contains?(first_line, ["Task", "Instruction", "Prompt"]) do
  ...
  defp analyze_prompt_purpose(prompt) do
    cond do
      String.contains?(prompt, ["analyze", "evaluate", "assess"]) ->
        %{primary_purpose: :analysis}

      String.contains?(prompt, ["generate", "create", "write"]) ->
        %{primary_purpose: :generation}

      String.contains?(prompt, ["transform", "convert", "rewrite"]) ->
        %{primary_purpose: :transformation}

      String.contains?(prompt, ["answer", "explain", "what", "why", "how"]) ->
        %{primary_purpose: :question_answering}
  ...
  defp calculate_complexity_score(prompt) do
    ...
    total_score =
      length_score * 0.2 + structure_score * 0.3 +
  ...
  defp calculate_parameter_complexity(prompt) do
    ...
    nested_params =
      Enum.count(parameters, fn [_, param] ->
  ...
  defp calculate_instruction_complexity(prompt) do
    ...
    # Cap at 0.8 to leave room for other factors
    |> min(0.8)
  end
  ...
  defp categorize_prompt(prompt) do
    ...
    categories =
      if String.contains?(prompt_lower, [
           "code",
           "program",
           "function",
           "algorithm",
    ...
    categories =
      if String.contains?(prompt_lower, ["translate", "language", "localization", "multilingual"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["customer", "support", "service", "help", "assistance"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["email", "message", "communication", "correspondence"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["education", "teach", "explain", "learn", "tutorial"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["research", "academic", "scholarly", "paper", "study"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["summarize", "summary", "brief", "overview"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["classify", "categorize", "tag", "organize"]) do
    ...
    categories =
      if String.contains?(prompt_lower, ["extract", "parse", "find", "identify"]) do
    ...
    case Enum.uniq(categories) do
      ...
      # Most specific first
      unique_categories -> Enum.reverse(unique_categories)
    end
  end
    ...
    models =
      cond do
        ...
        complexity_score > 0.7 and
            String.contains?(prompt_lower, ["analyze", "complex", "detailed", "comprehensive"]) ->
  ...
  defp analyze_prompt_characteristics(prompt) do
    ...
    parameters =
      Regex.scan(~r/\{\{(.*?)\}\}/, prompt) |> Enum.map(fn [_, param] -> String.trim(param) end)
  ...
  defp determine_primary_task_type(prompt) do
    ...
    cond do
      String.contains?(prompt_lower, ["analyze", "analysis", "examine", "evaluate", "assess"]) ->
        :analysis

      String.contains?(prompt_lower, ["write", "create", "generate", "compose", "draft"]) ->
        :generation

      String.contains?(prompt_lower, ["summarize", "summary", "brief", "condense"]) ->
        :summarization

      String.contains?(prompt_lower, ["translate", "convert", "transform"]) ->
        :transformation

      String.contains?(prompt_lower, ["classify", "categorize", "organize", "sort"]) ->
        :classification

      String.contains?(prompt_lower, ["extract", "find", "identify", "locate", "parse"]) ->
        :extraction

      String.contains?(prompt_lower, ["explain", "describe", "clarify", "elaborate"]) ->
        :explanation

      String.contains?(prompt_lower, ["compare", "contrast", "versus", "difference"]) ->
        :comparison

      String.contains?(prompt_lower, ["optimize", "improve", "enhance", "refine"]) ->
        :optimization

      String.contains?(prompt_lower, ["debug", "troubleshoot", "fix", "solve"]) ->
        :debugging
  ...
  defp format_best_practices_markdown(practices),
    do: "## Best Practices\n\n" <> Enum.join(practices, "\n")

  defp format_troubleshooting_markdown(_guide),
    do: "## Troubleshooting\n\nSee common issues section."

  defp format_examples_html(_examples), do: "<h2>Examples</h2><p>No examples available.</p>"

  defp generate_tutorial_id,
    do: "tutorial_" <> Base.encode16(:crypto.strong_rand_bytes(4), case: :lower)

  defp generate_template_id,
    do: "template_" <> Base.encode16(:crypto.strong_rand_bytes(4), case: :lower)
  ...
  defp format_api_documentation(docs, :markdown),
    do: "# API Documentation\n\n" <> docs.description

  defp format_api_documentation(_docs, :html),
    do: "<html><body><h1>API Documentation</h1></body></html>"
