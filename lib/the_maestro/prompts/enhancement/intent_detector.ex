defmodule TheMaestro.Prompts.Enhancement.IntentDetector do
  @moduledoc """
    ...
    %{
      ...
      file_operations: %{
        patterns: [
          ...
          ~r/\.\w{2,5}\b/,  # File extensions like .json, .txt, etc.
          ~r/\/[\w\/.-]+/,  # File paths
        ],
  ...
  def score_intent_category({category, config}, prompt) do
    ...
    base_confidence = case match_count do
      0 -> 0.0
      1 -> 0.3
      2 -> 0.5
      3 -> 0.7
    ...
    final_confidence = case match_count do
      0 -> base_confidence  # No boost for zero matches
    ...
    length_adjustment = case word_count do
  ...
  defp count_pattern_matches(patterns, prompt) do
    matched_info = Enum.reduce(patterns, {[], 0}, fn pattern, {matched, count} ->
      if Regex.match?(pattern, prompt) do
        pattern_string = inspect(pattern) |> String.slice(2..-3//1)  # Remove ~r/ and /i
