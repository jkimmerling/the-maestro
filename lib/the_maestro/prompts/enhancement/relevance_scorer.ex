defmodule TheMaestro.Prompts.Enhancement.RelevanceScorer do
  @moduledoc """
  ...
  defp calculate_entity_overlap(context_value, mentioned_entities) do
    if Enum.empty?(mentioned_entities) do
      ...
      overlap_count = Enum.count(mentioned_entities, fn entity ->
  ...
  defp calculate_domain_relevance(context_type, domain_indicators) do
    ...
    relevant_domains = Enum.filter(domain_indicators, fn domain ->
  ...
  defp calculate_freshness_factor(context_value) do
    case extract_timestamp(context_value) do
      ...
      timestamp ->
        ...
        cond do
          age_hours < 1 -> 0.2     # Very fresh
          age_hours < 24 -> 0.1    # Recent
          age_hours < 168 -> 0.05  # This week
          true -> 0.0              # Older
        end
    end
  end
  ...
  defp meets_relevance_threshold?(%ContextItem{relevance_score: score}) do
    score >= 0.2  # Minimum threshold for inclusion
  end
  ...
  defp stringify_context_value(%{__struct__: struct_name} = value) do
    case struct_name do
      TheMaestro.Prompts.Enhancement.Structs.EnvironmentalContext ->
        parts = []
        parts = if value.operating_system, do: ["OS: #{value.operating_system}" | parts], else: parts
        parts = if value.working_directory, do: ["Dir: #{value.working_directory}" | parts], else: parts
      ...
      TheMaestro.Prompts.Enhancement.Structs.ProjectStructureContext ->
        ...
        parts = if value.language_detection && length(value.language_detection) > 0, do: ["Languages: #{Enum.join(value.language_detection, ", ")}" | parts], else: parts
        parts = if value.framework_detection && length(value.framework_detection) > 0, do: ["Frameworks: #{Enum.join(value.framework_detection, ", ")}" | parts], else: parts
        Enum.reverse(parts) |> Enum.join(", ")
      
      TheMaestro.Prompts.Enhancement.Structs.CodeAnalysisContext ->
        parts = []
        parts = if value.relevant_files && length(value.relevant_files) > 0, do: ["Files: #{length(value.relevant_files)} found" | parts], else: parts
        parts = if value.dependencies && length(value.dependencies) > 0, do: ["Deps: #{length(value.dependencies)} found" | parts], else: parts
        Enum.reverse(parts) |> Enum.join(", ")
