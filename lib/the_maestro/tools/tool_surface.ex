defmodule TheMaestro.Tools.ToolSurface do
  ...
  def list(session_id, provider) when provider in [:openai, :gemini, :anthropic] do
    builtins =
      Manifest.list_builtin(provider)
      |> Enum.map(fn %{name: n, description: d} ->
        %{name: n, description: d, source: :builtin}
      end)
  ...
  def resolve_for_provider_decl(provider, session_id)
      when provider in [:openai, :gemini, :anthropic] do
  ...
  defp list_mcp_inventory(session_id, :openai) do
    MCPRegistry.to_openai_decls(session_id)
    |> Enum.flat_map(fn
      %{"name" => n} = m -> [%{name: n, description: m["description"], source: :mcp}]
      _ -> []
    end)
  end
