defmodule TheMaestro.TUI.SecurityConfirmation do
  ...
  defp display_risk_assessment(risk_assessment) do
    {color, symbol} =
      case risk_assessment.risk_level do
        :low -> {IO.ANSI.green(), "✓"}
  ...
  defp display_parameters(parameters) when map_size(parameters) > 0 do
    ...
    formatted =
      case Jason.encode(parameters, pretty: true) do
        {:ok, json} -> json
  ...
  defp get_user_choice(risk_level) do
    ...
    case {input, risk_level} do
      ...
      {"2", level} when level in [:low, :medium] ->
        :always_allow_tool

      {"3", level} when level in [:low, :medium] ->
        :always_trust_server

      {"4", level} when level in [:high, :critical] ->
        :block_tool
  ...
  defp create_confirmation_result(choice) do
    {decision, message, trust_updated} =
      case choice do
        :execute_once ->
