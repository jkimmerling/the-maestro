defmodule TheMaestro.MCP.Security.ServerTrust do
  ...
  def add_to_whitelist(%__MODULE__{} = trust, tool_name) do
    updated_whitelist =
      ...
      |> Enum.reject(&(&1 == tool_name))
      # Add to end
      |> Enum.concat([tool_name])

    %{trust | whitelist_tools: updated_whitelist, updated_at: DateTime.utc_now()}
  end
  ...
  def add_to_blacklist(%__MODULE__{} = trust, tool_name) do
    ...
    %{trust | blacklist_tools: updated_blacklist, updated_at: DateTime.utc_now()}
  end
  ...
  def update_trust_level(%__MODULE__{} = trust, new_level) do
    %{trust | trust_level: new_level, updated_at: DateTime.utc_now()}
  end
  ...
  def set_expiration(%__MODULE__{} = trust, expires_at) do
    %{trust | expires_at: expires_at, updated_at: DateTime.utc_now()}
  end
  ...
  def clear_expiration(%__MODULE__{} = trust) do
    %{trust | expires_at: nil, updated_at: DateTime.utc_now()}
  end
end
