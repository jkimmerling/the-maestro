defmodule TheMaestro.MCP.Security.AuditLogger do
  ...
  defmodule SecurityEvent do
    ...
    """
    @type event_type :: :tool_execution | :trust_granted | :trust_revoked | 
                        :access_denied | :confirmation_requested | :confirmation_response |
  ...
  """
  @spec log_tool_execution(String.t(), String.t(), String.t(), map(), atom(), atom(), String.t() | nil, map()) :: :ok
  def log_tool_execution(user_id, tool_name, server_id, parameters, risk_level, decision, reason \\ nil, metadata \\ %{}) do
    event = %SecurityEvent{
  ...
  """
  @spec log_trust_change(atom(), String.t(), String.t(), String.t() | nil, String.t(), map()) :: :ok
  def log_trust_change(event_type, user_id, server_id, tool_name \\ nil, granted_by, metadata \\ %{}) do
    event = %SecurityEvent{
      event_type: event_type,
      user_id: user_id,
      server_id: server_id,
  ...
  def log_access_denied(user_id, tool_name, server_id, reason, risk_level \\ :medium, metadata \\ %{}) do
    event = %SecurityEvent{
  ...
  def log_confirmation_request(user_id, tool_name, server_id, parameters, risk_level, metadata \\ %{}) do
    event = %SecurityEvent{
  ...
  """
  @spec log_confirmation_response(String.t(), String.t(), String.t(), atom(), atom(), map()) :: :ok
  ...
  def log_policy_violation(user_id, policy_name, violation_type, description, risk_level \\ :high, metadata \\ %{}) do
    event = %SecurityEvent{
  ...
  def log_anomaly_detected(user_id, anomaly_type, description, risk_level \\ :medium, metadata \\ %{}) do
    event = %SecurityEvent{
  ...
  defp format_text_message(event) do
    ...
    parts = if event.tool_name do
      ["Tool: #{event.tool_name}" | parts]
    else
      parts
    end
    
    parts = if event.server_id do
      ["Server: #{event.server_id}" | parts]
    else
      parts
    end
    
    parts = if event.risk_level do
      ["Risk: #{event.risk_level}" | parts]
    else
      parts
    end
    
    parts = if event.decision do
      ["Decision: #{event.decision}" | parts]
    else
      parts
    end
    
    parts = if event.reason do
