defmodule TheMaestro.MCP.Security.RiskAssessment do
  ...
  def severity_description(:critical),
    do: "Critical Risk - Dangerous operation that should be blocked"
  ...
  defp build_recommendations(risk_level, factors) do
    base_recommendations =
      case risk_level do
        ...
        :medium ->
          ["Review parameters before proceeding", "Consider if operation is necessary"]
        ...
        :critical ->
          ["DO NOT PROCEED", "This operation is too dangerous", "Find an alternative approach"]
      end
  ...
  defp factor_recommendation(:command_injection_risk),
    do: ["Sanitize input parameters", "Use safer alternatives"]

  defp factor_recommendation(:path_traversal_risk), do: ["Validate and sanitize file paths"]

  defp factor_recommendation(:sensitive_data_detected),
    do: ["Avoid including sensitive data", "Use environment variables"]
