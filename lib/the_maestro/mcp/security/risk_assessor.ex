defmodule TheMaestro.MCP.Security.RiskAssessor do
  ...
  @sensitive_paths [
    ...
    ".env", "config", "credentials", "private", "secret",
    "passwd", "shadow", "authorized_keys", "id_rsa", "id_dsa"
  ]
  
  # Dangerous command patterns
  @dangerous_commands [
    # Deletion commands
    "rm -rf", "rmdir", "del /s", "rd /s",
    # Permission changes
    "chmod 777", "chmod 666", "chown root", 
    # System modification
    "passwd", "sudo", "su -", "systemctl", "service",
    # Network tools that can be dangerous
    "wget", "curl.*sh", "curl.*bash", 
    # Archive extraction (can overwrite)
    "tar.*--absolute", "unzip.*-o",
    # Fork bomb and system overload
    ":()", "while true", "for((;;))",
    # Disk operations
    "dd if=", "fdisk", "mkfs", "format"
  ]
  ...
  defp add_command_based_factors(factors, parameters) do
    case get_command_parameter(parameters) do
      ...
      command ->
        ...
        new_factors = if is_dangerous_command?(command) do
          [:destructive_command | new_factors]
        else
          new_factors
        end
        
        new_factors = if has_command_injection_risk?(command) do
          [:command_injection_risk | new_factors]
        else
          new_factors
        end
        
        new_factors = if is_system_modification_command?(command) do
  ...
  defp add_network_based_factors(factors, parameters) do
    case get_network_parameter(parameters) do
      ...
      url ->
        ...
        new_factors = if is_external_service?(url) do
          [:external_service | new_factors]
        else
          new_factors
        end
        
        new_factors = if is_insecure_protocol?(url) do
  ...
  defp factor_weight(:safe_path), do: -0.05  # Actually reduces risk
  ...
  defp factor_weight(:system_access), do: 0.4  # Add this one
  defp factor_weight(_), do: 0.05  # Unknown factors get minimal weight
  ...
  defp has_high_risk_factors?(factors) do
    high_risk_factors = [
      :sensitive_path, :path_traversal_risk, :system_modification,
      :destructive_operation, :sensitive_data_detected
    ]
