defmodule TheMaestro.MCP.Security.SecureExecutor do
  ...
  def execute_secure(tool_name, parameters, context) do
    ...
    with {:ok, effective_policy} <- evaluate_policy(context),
         {:ok, permission_checks} <- check_permissions(tool_name, parameters, context, effective_policy),
         {:ok, sanitized_params, sanitization_warnings} <- sanitize_parameters(tool_name, parameters, context),
         ...
         {:ok, execution_result} <- execute_if_authorized(tool_name, sanitized_params, context, confirmation_result) do
  ...
  def execute_headless(tool_name, parameters, context, policy_settings \\ %{}) do
    ...
    with {:ok, sanitized_params, sanitization_warnings} <- 
         ...
         {:ok, execution_result} <- 
           execute_if_authorized(tool_name, sanitized_params, headless_context, confirmation_result) do
      ...
      error -> error  # Pass through other errors
    end
  end
  ...
  defp check_permissions(tool_name, parameters, context, effective_policy) do
    ...
    permission_checks = permission_checks ++ check_command_permissions(tool_name, parameters, permissions)
  ...
  defp detect_anomalies(tool_name, parameters, context) do
    ...
    case AnomalyDetector.analyze_context(context) do
      {:ok, anomalies} -> 
        ...
        blocking_anomalies = Enum.filter(anomalies, fn anomaly ->
      ...
      {:error, reason} ->
        ...
        {:ok, []}  # Continue with empty anomaly list if detection fails
    end
  end
  ...
  defp extract_file_paths(parameters) do
    ...
    path_params = ["path", "file", "filename", "input_file", "output_file", "source", "destination"]
    
    paths = Enum.reduce(path_params, paths, fn param, acc ->
      case Map.get(parameters, param) do
        path when is_binary(path) ->
          # Determine access type based on parameter name
          access_type = case param do
            name when name in ["output_file", "destination"] -> :write
            "path" -> :read  # Default assumption
            _ -> :read
          end
  ...
  defp extract_network_endpoints(parameters) do
    ...
    endpoints = Enum.reduce(network_params, endpoints, fn param, acc ->
      case Map.get(parameters, param) do
        endpoint when is_binary(endpoint) ->
          [{endpoint, :outbound} | acc]  # Assume outbound by default
        _ -> acc
      end
    end)
  ...
  defp extract_commands(tool_name, parameters) do
    ...
    commands = Enum.reduce(command_params, commands, fn param, acc ->
      case Map.get(parameters, param) do
  ...
  defp evaluate_security(tool_name, parameters, context) do
    ...
    confirmation_request = ConfirmationEngine.evaluate_confirmation_requirement(
      tool, 
      parameters, 
      security_context
    )
  ...
  defp handle_headless_confirmation(confirmation_request, policy_settings \\ %{}) do
    confirmation_result = ConfirmationEngine.handle_headless_security(
  ...
  defp handle_interactive_confirmation(confirmation_request, context) do
    ...
    confirmation_result = ConfirmationEngine.process_confirmation_choice(
      confirmation_request,
      choice,
      confirmation_context
    )
  ...
  defp simulate_user_choice(risk_level) do
    ...
    case risk_level do
      :low -> :execute_once
      :medium -> :execute_once
      :high -> :cancel  # Simulate cautious user
      :critical -> :cancel
    end
  end
