defmodule TheMaestro.MCP.Security.ParameterSanitizer do
  ...
  @script_injection_patterns ["<script", "</script", "javascript:", "vbscript:", "onload=", "onerror="]
  @sql_injection_patterns ["' or ", "\" or ", "union select", "drop table", "insert into", "delete from"]
  ...
  defp sanitize_parameter(key, value, tool_name, result, options) do
    {sanitized_value, warnings} = case {String.downcase(to_string(key)), value} do
  ...
  defp valid_path_format?(path) do
    ...
    byte_size(path) < 4096 and  # Reasonable length limit
    not String.contains?(path, "\0") and  # No null bytes
  ...
  defp sanitize_command_basic(command) do
    command
    |> String.trim()
    |> String.slice(0, 1024)  # Limit command length
  end
  ...
  defp detect_string_security_issues(value, key) do
    ...
    warnings = if has_script_injection_patterns?(value) do
      ["Parameter '#{key}' contains potential script injection patterns" | warnings]
    else
      warnings
    end
    
    warnings = if has_sql_injection_patterns?(value) do
      ["Parameter '#{key}' contains potential SQL injection patterns" | warnings]
    else
      warnings
    end
    
    warnings = if String.contains?(value, "\0") do
      ...
      |> Enum.map(fn {item, index} ->
           case item do
             string when is_binary(string) ->
               ...
               warnings = if has_path_traversal?(string) do
