defmodule TheMaestro.MCP.Security.AnomalyDetector do
  ...
  @type anomaly_type :: :usage_pattern | :access_pattern | :temporal_pattern | 
                        :parameter_pattern | :resource_pattern | :behavioral_pattern
  ...
  """
  @spec update_anomaly_status(String.t(), anomaly_status(), String.t()) :: :ok | {:error, String.t()}
  ...
  def init(opts) do
    ...
    :timer.send_interval(300_000, self(), :update_baselines)  # 5 minutes
    
    # Schedule anomaly cleanup
    :timer.send_interval(3600_000, self(), :cleanup_resolved_anomalies)  # 1 hour
  ...
  defp detect_usage_pattern_anomalies(event, state) do
    ...
    if user_id do
      recent_user_events = get_recent_user_events(state, user_id, 60) # last minute
      tool_count = recent_user_events |> Enum.map(&Map.get(&1, :tool_name)) |> Enum.uniq() |> length()
      
      anomalies = if tool_count > state.thresholds.max_tools_per_minute do
        [create_anomaly(:usage_pattern, :medium, "Excessive tool usage rate", %{
  ...
  defp detect_parameter_pattern_anomalies(event, state) do
    ...
    |> Enum.flat_map(fn pattern ->
      ...
      if length(matches) > 0 do
        [create_anomaly(:parameter_pattern, pattern.severity, "Suspicious parameter pattern: #{pattern.name}", %{
  ...
  defp detect_temporal_pattern_anomalies(event, state) do
    ...
    if is_off_hours?(timestamp) and user_id do
      ...
      if normal_hours_activity > state.thresholds.off_hours_score_threshold do
        anomalies = [create_anomaly(:temporal_pattern, :medium, "Off-hours access by typically daytime user", %{
          timestamp: timestamp,
    ...
    if user_id do
      recent_events = get_recent_user_events(state, user_id, 300) # last 5 minutes
      event_rate = length(recent_events) / 5.0 # events per minute
      ...
      if event_rate > baseline_rate * state.thresholds.burst_activity_threshold do
        anomalies = [create_anomaly(:temporal_pattern, :high, "Burst activity detected", %{
  ...
  defp detect_access_pattern_anomalies(event, state) do
    ...
    if file_path do
      ...
      if length(matches) > 0 do
        anomalies = [create_anomaly(:access_pattern, :high, "Sensitive file access attempt", %{
          file_path: file_path,
          matches: matches
        }, event) | anomalies]
      end
  ...
  defp detect_resource_pattern_anomalies(event, state) do
    ...
    if user_id != nil and map_size(resource_usage) > 0 do
      ...
      if cpu_usage > baseline_cpu * state.thresholds.cpu_usage_anomaly_threshold do
        anomalies = [create_anomaly(:resource_pattern, :medium, "Excessive CPU usage", %{
      ...
      if memory_usage > baseline_memory * state.thresholds.memory_usage_anomaly_threshold do
        anomalies = [create_anomaly(:resource_pattern, :medium, "Excessive memory usage", %{
    ...
    if user_id and tool_name do
      ...
      if tool_name not in user_tools do
        ...
        if tool_novelty_score > state.thresholds.new_tool_usage_threshold do
          anomalies = [create_anomaly(:behavioral_pattern, :low, "New tool usage", %{
            tool_name: tool_name,
      ...
      if deviation_score > state.thresholds.user_behavior_deviation_threshold do
        anomalies = [create_anomaly(:behavioral_pattern, :medium, "User behavior deviation", %{
  ...
  defp sanitize_parameters_for_logging(parameters) do
    ...
    |> Enum.map(fn {key, value} ->
      sanitized_value = if String.length(to_string(value)) > 100 do
  ...
  defp is_off_hours?(timestamp) do
    hour = timestamp.hour
    hour < 6 or hour > 22  # Consider 6 AM to 10 PM as normal hours
  end
  ...
  defp calculate_user_baseline(events) do
    if length(events) == 0 do
      ...
      %{
        common_tools: events |> Enum.map(&Map.get(&1, :tool_name)) |> Enum.frequencies() |> Map.keys(),
        avg_events_per_minute: length(events) / 60.0,  # Assuming events from last hour
        avg_cpu_usage: events |> Enum.map(&get_in(&1, [:resource_usage, :cpu_percent])) |> Enum.reject(&is_nil/1) |> average(),
        avg_memory_usage: events |> Enum.map(&get_in(&1, [:resource_usage, :memory_mb])) |> Enum.reject(&is_nil/1) |> average(),
  ...
  defp calculate_normal_hours_ratio(events) do
    return_hours_ratio = if length(events) == 0 do
      0.5
    else
      normal_hours_count = 
        events
  ...
  defp cleanup_old_anomalies(state) do
    cutoff = DateTime.add(DateTime.utc_now(), -7 * 24 * 60 * 60, :second)  # 7 days ago
