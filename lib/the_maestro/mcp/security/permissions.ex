defmodule TheMaestro.MCP.Security.Permissions do
  ...
  defstruct file_system: %{read: [], write: [], execute: []},
            network: %{
              outbound: [],
              inbound: [],
              blocked_domains: [],
    ...
    new_net = %{
      ...
      blocked_domains:
        merge_permission_lists(
          current_net.blocked_domains,
          Map.get(additional, :blocked_domains, [])
        ),
      allowed_protocols:
        merge_permission_lists(
          current_net.allowed_protocols,
          Map.get(additional, :allowed_protocols, [])
        )
    }
  ...
  defp merge_system_permissions(permissions, additional) do
    ...
    new_sys = %{
      environment_vars:
        merge_permission_lists(
          current_sys.environment_vars,
          Map.get(additional, :environment_vars, [])
        ),
      commands: merge_permission_lists(current_sys.commands, Map.get(additional, :commands, [])),
      blocked_commands:
        merge_permission_lists(
          current_sys.blocked_commands,
          Map.get(additional, :blocked_commands, [])
        )
    }
  ...
  defp validate_file_system_permissions(fs_perms, errors) do
    errors =
      if not is_list(fs_perms.read),
        do: ["file_system.read must be a list" | errors],
        else: errors

    errors =
      if not is_list(fs_perms.write),
        do: ["file_system.write must be a list" | errors],
        else: errors

    errors =
      if not is_list(fs_perms.execute),
        do: ["file_system.execute must be a list" | errors],
        else: errors
  ...
  defp validate_network_permissions(net_perms, errors) do
    errors =
      if not is_list(net_perms.outbound),
        do: ["network.outbound must be a list" | errors],
        else: errors

    errors =
      if not is_list(net_perms.inbound),
        do: ["network.inbound must be a list" | errors],
        else: errors

    errors =
      if not is_list(net_perms.blocked_domains),
        do: ["network.blocked_domains must be a list" | errors],
        else: errors

    errors =
      if not is_list(net_perms.allowed_protocols),
        do: ["network.allowed_protocols must be a list" | errors],
        else: errors
  ...
  defp validate_system_permissions(sys_perms, errors) do
    errors =
      if not is_list(sys_perms.environment_vars),
        do: ["system.environment_vars must be a list" | errors],
        else: errors

    errors =
      if not is_list(sys_perms.commands),
        do: ["system.commands must be a list" | errors],
        else: errors

    errors =
      if not is_list(sys_perms.blocked_commands),
        do: ["system.blocked_commands must be a list" | errors],
        else: errors
  ...
  defp validate_resource_limits(resources, errors) do
    errors =
      if not is_integer(resources.max_cpu_percent),
        do: ["resources.max_cpu_percent must be an integer" | errors],
        else: errors

    errors =
      if not is_integer(resources.max_memory_mb),
        do: ["resources.max_memory_mb must be an integer" | errors],
        else: errors

    errors =
      if not is_integer(resources.max_execution_seconds),
        do: ["resources.max_execution_seconds must be an integer" | errors],
        else: errors

    errors =
      if not is_integer(resources.max_file_size_mb),
        do: ["resources.max_file_size_mb must be an integer" | errors],
        else: errors
