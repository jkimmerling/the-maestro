defmodule TheMaestro.MCP.Security.ConfirmationEngine do
  ...
  @type confirmation_choice :: :execute_once | :always_allow_tool | :always_trust_server | :block_tool | :cancel
  ...
  def evaluate_confirmation_requirement(tool, parameters, context) do
    ...
    requires_trust_confirmation = GenServer.call(
      trust_manager_pid, 
      {:requires_confirmation, tool, parameters, context}
    )
    
    # Determine overall confirmation requirement
    requires_confirmation = determine_confirmation_requirement(
      risk_assessment, 
  ...
  """
  @spec process_confirmation_choice(ConfirmationRequest.t(), confirmation_choice(), confirmation_context()) :: ConfirmationResult.t()
  ...
  defp build_confirmation_reason(risk_assessment, requires_trust_confirmation) do
    ...
    reasons = if RiskAssessment.requires_confirmation?(risk_assessment) do
      ["Risk level: #{risk_assessment.risk_level}" | reasons]
    else
      reasons
    end
    
    reasons = if requires_trust_confirmation do
