defmodule TheMaestro.MCP.Transport.HTTP do
  ...
  def handle_call({:send_message, message}, from, state) do
    case send_streaming_request(state, message, from) do
      {:ok, request_id} ->
        new_state = %{state | 
          active_requests: Map.put(state.active_requests, request_id, from)
        }
        {:noreply, new_state}
  ...
  defp find_balanced_json(<<char::utf8, rest::binary>>, brace_count, pos, acc) when brace_count > 0 do
