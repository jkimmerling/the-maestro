      ...
      <div class="min-h-screen bg-black text-amber-400 font-mono relative overflow-hidden">
        <div class="container mx-auto px-6 py-8">
          <div class="flex justify-between items-center mb-6 border-b border-amber-600 pb-4">
            <h1 class="text-3xl md:text-4xl font-bold text-amber-400 glow tracking-wider">&gt;&gt;&gt; {@page_title} &lt;&lt;&lt;</h1>
            <div class="space-x-2">
              <%= if @live_action == :edit do %>
                <button phx-click="delete" data-confirm="Delete this agent?" class="px-3 py-1 rounded btn-red">DELETE</button>
              <% end %>
              <.link navigate={return_path(@return_to, @agent)} class="px-3 py-1 rounded btn-amber" data-hotkey-seq="g i" data-hotkey-label="Back">BACK</.link>
            </div>
          </div>
          ...
          <.form for={@form} id="agent-form" phx-change="validate" phx-submit="save">
            <div class="terminal-card terminal-border-amber p-6 space-y-4">
              ...
              <.input field={@form[:model_id]} type="select" label="Model" options={@model_options} prompt="Auto-select from provider" />
              ...
              <footer class="space-x-2">
                <button type="submit" class="px-4 py-2 rounded btn-green" phx-disable-with="Saving...">Save Agent</button>
                <.link navigate={return_path(@return_to, @agent)} class="px-4 py-2 rounded btn-amber">Cancel</.link>
              </footer>
            </div>
          </.form>
        </div>
