  ...
  client when not is_tuple(client) ->
    ...
    case headers_middleware do
      {Tesla.Middleware.Headers, :call, [headers]} ->
        ...
        IO.puts(
          "   Claude Code Beta: #{if beta_header && elem(beta_header, 1) =~ "claude-code-20250219", do: "✓ Present", else: "✗ Missing"}"
        )
    ...
    test_request = %{
      ...
      messages: [
        %{
          role: "user",
          content:
            "Please confirm you are Claude Code and respond with: 'OAuth authentication successful'"
        }
      ]
    }
