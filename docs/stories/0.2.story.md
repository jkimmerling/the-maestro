...
- [ ] **Task 1.1: Req Dependency Integration** (AC-1)
  - [ ] **MANDATORY: Add Req dependency to mix.exs**
    - [ ] Add `{:req, "~> 0.5.0"}` to dependencies
  ...
  - [ ] **MANDATORY: Req Global Configuration** (AC-3)
    - [ ] Configure Req to use existing Finch pools by default
    - [ ] Set global Req options for retry, timeout, and logging
    - [ ] Create base Req configuration that all providers inherit
    - [ ] Document Req configuration patterns for provider implementations

- [ ] **Task 1.2: Base Req Client Factory** (AC-3)
  - [ ] **MANDATORY: Create `lib/the_maestro/providers/http/req_client_factory.ex`**
    - [ ] **MANDATORY: Complete @spec declarations for factory functions:**
      ```elixir
      ...
      ```
    - [ ] **MANDATORY: Provider-specific client creation patterns**
      ```elixir
      ...
      ```
    - [ ] **MANDATORY: Unified authentication handling**
      - [ ] OAuth Bearer token injection for OAuth sessions
      - [ ] API key authentication for API key sessions  
      - [ ] Provider-specific header patterns (Anthropic x-api-key, OpenAI Bearer, etc.)
...
- [ ] **Task 2.1: Req Streaming Adapter** (AC-2)
  - [ ] **MANDATORY: Create `lib/the_maestro/providers/http/streaming_adapter.ex`**
    - [ ] **MANDATORY: Complete @spec declarations for streaming functions:**
      ```elixir
    ...
    - [ ] **MANDATORY: SSE parsing with error recovery**
      - [ ] Parse Server-Sent Event format from Req stream chunks
