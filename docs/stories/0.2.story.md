...
├── providers/
│   └── [provider_modules]        # Use Req client factory + transport adapter
└── workers/
    └── token_refresh_worker.ex   # Update docs to reflect Req (remove Tesla refs)
```

**Files to Remove (after validation):**
```
lib/the_maestro/providers/
└── [tesla_middleware]            # Tesla-specific middleware (already removed)
```
