...
- [ ] **Task 1: Research Anthropic API header requirements and llxprt reference implementation** (AC: 2)
  - [ ] Use Archon MCP to research Anthropic API authentication best practices
  - [ ] Study `llxprt` reference implementation for exact header patterns
  - [ ] Document required header order and values for Anthropic API

- [ ] **Task 2: Create Anthropic API Key authentication middleware** (AC: 1, 2)
  - [ ] Create new Tesla middleware module for Anthropic API key authentication
  - [ ] Implement precise header injection in exact order specified
  - [ ] Load API key from application configuration
  - [ ] Ensure middleware integrates properly with existing Tesla client structure

- [ ] **Task 3: Extend `build_client/1` function to support Anthropic API Key mode** (AC: 1, 2)
  - [ ] Modify existing `build_client/1` to detect and handle API key authentication
  - [ ] Update provider configuration to include authentication type
  - [ ] Integrate Anthropic API Key middleware into Tesla client stack
  - [ ] Ensure backwards compatibility with existing client functionality

- [ ] **Task 4: Implement configuration management for API keys** (AC: 1, 2)
  - [ ] Add configuration fields for Anthropic API key in environment configs
  - [ ] Implement secure configuration loading from environment variables
  - [ ] Add validation for required configuration values
  - [ ] Document configuration requirements for deployment

- [ ] **Task 5: Write comprehensive tests for Anthropic API Key authentication** (AC: 3)
  - [ ] Test Tesla client creation with `:anthropic` provider and API key mode
  - [ ] Test exact header order and values in outgoing requests
  - [ ] Test configuration loading and validation
  - [ ] Add integration test making real API call to Anthropic (with test key)
  - [ ] Test error handling for missing or invalid API keys

- [ ] **Task 6: Update type specifications and documentation** (AC: All)
  - [ ] Add type specifications for new authentication modes
  - [ ] Update module documentation with API key authentication examples
  - [ ] Document configuration requirements and environment setup
  - [ ] Add code examples for API key usage
