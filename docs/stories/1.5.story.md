...
- [ ] **Task 1: Research OpenAI API authentication patterns from llxprt reference** (AC: 1, 2)
  - [ ] **MANDATORY: Research OpenAI Bearer authentication patterns before implementation**
    - [ ] Query Archon for OpenAI Bearer authentication best practices: `archon:perform_rag_query(query="OpenAI Bearer authentication best practices", match_count=3)`
    - [ ] Query Archon for Tesla middleware OAuth authentication examples: `archon:search_code_examples(query="Tesla HTTP client Bearer token authentication examples", match_count=3)`
  - [ ] Extract exact Bearer token header requirements from OpenAI API documentation
  - [ ] Document differences between OpenAI and Anthropic authentication headers
  - [ ] Validate OpenAI Bearer token usage patterns with existing Tesla + Finch infrastructure

- [ ] **Task 2: Extend Client.build_client/1 for OpenAI authentication** (AC: 1, 2)
  - [ ] Add OpenAI provider configuration to existing `build_client/1` function
  - [ ] Implement OpenAI-specific middleware for Bearer token injection
  - [ ] **MANDATORY: Add comprehensive @spec declarations for ALL OpenAI authentication functions:**
  ...
  - [ ] **MANDATORY: Define required structs for OpenAI client configuration:**
  ...
  - [ ] Ensure OpenAI tokens are loaded from configuration (OPENAI_API_KEY environment variable)
  - [ ] Add proper error handling for missing API keys and organization IDs

- [ ] **Task 3: Add OpenAI provider configuration** (AC: 1)
  ...
  - [ ] Add OpenAI configuration to existing provider configuration patterns
  - [ ] Add OpenAI base URL and connection pool configuration
  - [ ] Update Finch connection pools to include OpenAI endpoint
  - [ ] Add OpenAI API key and organization ID configuration loading
...
- [ ] **Task 4: Write comprehensive tests** (AC: 1, 2, 3)
  ...
  - [ ] Test OpenAI client creation with Bearer token middleware
  - [ ] Test header injection in exact order specified in acceptance criteria
  - [ ] Test error handling for missing API keys and organization IDs
  - [ ] Test integration with existing Tesla + Finch HTTP infrastructure
  - [ ] Test OpenAI vs Anthropic authentication header differences
  - [ ] Test actual OpenAI API call with valid 200 OK response

- [ ] **Task 5: Update configuration and documentation** (AC: All)
  - [ ] **MANDATORY: Add comprehensive @spec declarations for configuration functions:**
    - `@spec validate_openai_environment_config() :: {:ok, map()} | {:error, term()}`
    - `@spec merge_openai_provider_configs(map()) :: {:ok, map()} | {:error, term()}`
  - [ ] Update runtime.exs configuration for OpenAI API key and organization ID
  - [ ] Add OpenAI configuration examples and environment variable documentation
  - [ ] Document OpenAI authentication workflow and header requirements
  - [ ] Update Client module documentation with OpenAI authentication examples
  - [ ] Add code examples for OpenAI-authenticated API calls
...
### Agent Model Used
_This section populated by development agent during implementation_
...
## QA Results
_Results from QA Agent review of completed story implementation_