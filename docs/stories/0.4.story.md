...
## Status
**Approved - Epic 0 Anthropic Modular Implementation**
...
- [ ] **Task 1.1: Provider Directory Structure Creation** (AC-1)
  - [ ] **MANDATORY: Create complete Anthropic provider module structure**
    ...
    ```
  - [ ] **Each module must implement required provider behaviors**
  - [ ] **Validation: Directory structure matches universal provider pattern from Story 0.1**

- [ ] **Task 1.2: OAuth Module Implementation** (AC-1, AC-2)
  - [ ] **MANDATORY: Implement `TheMaestro.Providers.Anthropic.OAuth`**
  - [ ] **OAuth behavior implementation:**
    ...
    ```
  - [ ] **Manual OAuth flow with no callback server dependency**
  - [ ] **JSON token exchange format (not form-encoded)**
  - [ ] **Integration with Anthropic OAuth endpoints**
  - [ ] **Validation: OAuth module compiles and implements all required callbacks**
  - [ ] **MANDATORY: Streaming endpoint & headers for OAuth (AC-5)**
    - [ ] Endpoint: `https://api.anthropic.com/v1/messages` with `stream: true`
    - [ ] Headers: `Authorization: Bearer <access_token>`, `anthropic-version: 2023-06-01`, `anthropic-beta: <flags>`, `Content-Type: application/json`

- [ ] **Task 1.3: API Key Module Implementation** (AC-1)
  - [ ] **MANDATORY: Implement `TheMaestro.Providers.Anthropic.ApiKey`**
  ...
  - [ ] **MANDATORY: Endpoint & headers (AC-5, AC-6)**
    - [ ] Endpoint: `https://api.anthropic.com/v1/messages` with `stream: true`
    - [ ] Headers: `x-api-key`, `anthropic-version: 2023-06-01`, `anthropic-beta: <flags>`, `Content-Type: application/json`

- [ ] **Task 1.4: Models Module Implementation** (AC-1)
  - [ ] **MANDATORY: Implement `TheMaestro.Providers.Anthropic.Models`**
  - [ ] **Models behavior implementation:**
    ...
    ```
  - [ ] **Claude model listing (claude-3-5-sonnet, claude-3-opus, claude-3-haiku)**
...
- [ ] **Task 2.1: Streaming Module Implementation** (AC-3)
  - [ ] **MANDATORY: Implement `TheMaestro.Providers.Anthropic.Streaming`**
  - [ ] **Streaming behavior implementation:**
    ...
    ```
  - [ ] **Integration with existing `TheMaestro.Streaming.AnthropicHandler`**
  ...
  - [ ] **Mode-aware header construction (AC-5)**
    - [ ] OAuth: `Authorization: Bearer ...`
    - [ ] API key: `x-api-key: ...`
    - [ ] Shared: `anthropic-version`, `anthropic-beta`, `Content-Type: application/json`

- [ ] **Task 2.2: Universal Streaming Adapter** (AC-3)
  - [ ] **MANDATORY: Create Req streaming adapter for Anthropic**
  - [ ] **Adapter feeds `TheMaestro.Streaming.parse_stream/3` with proper enumerable**
  - [ ] **Consistent SSE parsing across all providers (OpenAI, Anthropic, Gemini)**
  - [ ] **Error handling and recovery within streaming adapter**
...
- [ ] **Task 3.2: API Key E2E Streaming Test** (AC-6)
  - [ ] Create `scripts/test_anthropic_api_key_streaming_e2e.exs`
  - [ ] Create named API key session via `TheMaestro.Provider`
  - [ ] Validate model listing works with API key session
  - [ ] Stream standardized prompt; process with universal streaming parser
  - [ ] Validate complete response, usage stats, and header presence
...
- [ ] **Task 3.1: OAuth URL Generation** (AC-2)
  - [ ] **MANDATORY: Implement authorization URL generation with PKCE**
  - [ ] **Anthropic OAuth 2.0 endpoint integration:**
    ```
...
- [ ] **Task 3.2: Manual Code Entry Handler** (AC-2)
  - [ ] **MANDATORY: Implement manual authorization code input handling**
  - [ ] **User experience for code entry (similar to Story 1.4 patterns)**
  - [ ] **Code validation and sanitization**
  - [ ] **State parameter verification for security**
  - [ ] **Validation: Manual code entry works securely and reliably**

- [ ] **Task 3.3: JSON Token Exchange** (AC-2)
  - [ ] **MANDATORY: Implement JSON-formatted token exchange**
  - [ ] **Anthropic token endpoint integration with Req client:**
    ```elixir
...
- [ ] **Task 4.1: Provider Registration** (AC-1)
  - [ ] **MANDATORY: Register Anthropic provider with dynamic resolver**
...
- [ ] **Task 4.2: Generic Interface Compliance** (AC-1)
...
- [ ] **Task 4.3: Named Sessions Support** (AC-1)
