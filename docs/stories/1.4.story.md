...
- [ ] **Task 2: Extend Client.build_client/1 for OAuth authentication** (AC: 1)
  - [ ] Add OAuth authentication support to existing `build_middleware/2` function
  - [ ] Implement OAuth token retrieval from saved_authentications table
  - [ ] Create middleware for Bearer token injection with correct header format
  - [ ] **MANDATORY: Add comprehensive @spec declarations for ALL OAuth authentication functions:**
  ...
  - [ ] **MANDATORY: Define required structs for OAuth client configuration:**
  ...
  - [ ] Ensure OAuth tokens are loaded from encrypted database storage
  - [ ] Add proper error handling for missing or expired OAuth tokens

- [ ] **Task 3: Create Oban worker for token refresh** (AC: 2)
