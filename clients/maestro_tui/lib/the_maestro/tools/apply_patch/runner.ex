defmodule TheMaestro.Tools.ApplyPatch.Runner do
  ...
  @type result :: {:ok, %{added: [String.t()], modified: [String.t()], deleted: [String.t()], details: [map()]}} | {:error, String.t()}
  ...
  defp apply_hunks(hunks, base) do
    Enum.reduce_while(hunks, {:ok, %{added: [], modified: [], deleted: [], details: []}}, fn h, {:ok, acc} -> handle_hunk(h, acc, base) end)
  end
  ...
  defp handle_hunk({:add, path, contents}, acc, base) do
    ...
    {:cont, {:ok, acc |> Map.update!(:added, &(&1 ++ [abs])) |> Map.update!(:details, &(&1 ++ [det]))}}
  end
  defp handle_hunk({:delete, path}, acc, base) do
    ...
    {:cont, {:ok, acc |> Map.update!(:deleted, &(&1 ++ [abs])) |> Map.update!(:details, &(&1 ++ [det]))}}
  end
  defp handle_hunk({:update, path, move_to, chunks}, acc, base) do
    ...
    case apply_update(src, dst, chunks) do
      {:ok, prev, newc} ->
        ...
        det = %{file_path: dst, change_type: "update", diff: udiff, summary: sum, move_to: if(move_to, do: dst, else: nil)}
        {:cont, {:ok, acc |> Map.update!(:modified, &(&1 ++ [dst])) |> Map.update!(:details, &(&1 ++ [det]))}}
  ...
  defp apply_update(src, dst, chunks) do
    ...
    with {:ok, original} <- read_or_empty(dst), {:ok, new_content} <- compute_new_contents(original, chunks) do
  ...
  defp read_or_empty(path) do
    case File.read(path) do
      {:ok, bin} -> {:ok, bin}
      {:error, :enoent} -> {:ok, ""}
      {:error, r} -> {:error, :file.format_error(r) |> List.to_string()}
    end
  end
  ...
  defp compute_new_contents(original, chunks) do
    ...
    {final, status} = Enum.reduce_while(chunks, {lines, :ok}, fn ch, {cur, :ok} -> case replace_chunk(cur, ch) do {:ok, cur2} -> {:cont, {cur2, :ok}}; {:error, reason} -> {:halt, {cur, {:error, reason}}} end end)
    case status do :ok -> {:ok, Enum.join(final, "\n")}; {:error, r} -> {:error, r} end
  end

  defp replace_chunk(lines, %{change_context: ctx, old_lines: olds, new_lines: news, is_end_of_file: eof?}) do
    start_pos = case ctx do nil -> 0; ctx_line -> SeekSequence.seek_sequence(lines, [ctx_line], 0, false) || 0 end
    pos = SeekSequence.seek_sequence(Enum.drop(lines, start_pos), olds, 0, eof?)
    case pos do
      nil -> {:error, "context mismatch"}
      idx ->
        i = start_pos + idx
        {head, rest} = Enum.split(lines, i)
        {_, tail} = Enum.split(rest, length(olds))
        {:ok, head ++ news ++ tail}
    end
  end

  @spec format_summary(%{added: [String.t()], modified: [String.t()], deleted: [String.t()]}, String.t()) :: String.t()
  def format_summary(%{added: a, modified: m, deleted: d}, base) do
    rel = fn p -> case Path.relative_to(p, base) do ^p -> p; r -> r end end
