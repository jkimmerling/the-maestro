defmodule TheMaestro.Prompts.EngineeringTools.CollaborationToolsTest do
  ...
  describe "stop_real_time_sync/1" do
    setup do
      {:ok, session} =
        CollaborationTools.create_session(["user1", "user2"], %{real_time_sync: true})
  ...
  describe "create_comment/4" do
    ...
    test "creates a comment on prompt content", %{session: session} do
      comment_data =
        CollaborationTools.create_comment(
          session,
          "user1",
  ...
  describe "resolve_comment/3" do
    ...
    test "resolves an active comment", %{session: session, comment: comment} do
      {:ok, resolved_comment} =
        CollaborationTools.resolve_comment(
          session,
          comment.comment_id,
          "user2"
        )
  ...
  describe "track_change/4" do
    ...
    test "tracks prompt changes", %{session: session} do
      change =
        CollaborationTools.track_change(
          session,
          "user1",
          :content_update,
          %{
  ...
  describe "create_approval_request/3" do
    setup do
      {:ok, session} =
        CollaborationTools.create_session(["user1", "user2"], %{approval_workflow: true})
    ...
    test "creates approval request", %{session: session} do
      request =
        CollaborationTools.create_approval_request(
          session,
          "user1",
          %{
  ...
  describe "approve_request/3" do
    ...
    test "approves pending request", %{session: session, request: request} do
      {:ok, approved_request} =
        CollaborationTools.approve_request(
          session,
          request.request_id,
          "user2"
        )
  ...
  describe "handle_conflict/4" do
    ...
    test "resolves conflict automatically when configured", %{session: session} do
      ...
      resolution =
        CollaborationTools.handle_conflict(
          session,
          :content_conflict,
          conflict_data,
      ...
      # Latest timestamp
      assert resolution.resolved_content == "Version B"
