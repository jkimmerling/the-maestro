defmodule TheMaestro.Prompts.EngineeringTools.VersionControlTest do
  ...
  describe "create_branch/3" do
    ...
    test "creates a branch from specific source branch", %{repo: repo} do
      ...
      {:ok, updated_repo} = VersionControl.create_branch(repo_with_dev, "feature", %{source_branch: "dev"})
  ...
  describe "merge_branch/3" do
    setup do
      ...
      {:ok, repo_with_staged} = VersionControl.stage_changes(repo_with_changes, :all)
      {:ok, repo_with_commit} = VersionControl.commit(repo_with_staged, "Add feature", "test_user")
    ...
    test "detects merge conflicts", %{repo: repo} do
      ...
      {:ok, repo_with_main_commit} = VersionControl.commit(repo_with_main_staged, "Main changes", "test_user")
      
      {:ok, result} = VersionControl.merge_branch(repo_with_main_commit, "feature", %{strategy: :auto})
  ...
  describe "create_tag/3" do
    setup do
      ...
      repo_with_changes = %{repo | working_directory: working_changes}
      {:ok, repo_with_staged} = VersionControl.stage_changes(repo_with_changes, :all)
      {:ok, repo_with_commit} = VersionControl.commit(repo_with_staged, "Initial commit", "test_user")
    ...
    test "prevents duplicate tag names", %{repo: repo} do
      ...
      {:error, reason} = VersionControl.create_tag(repo_with_tag, "v1.0.0", latest_commit.commit_id)
  ...
  describe "get_commit_history/2" do
    setup do
      ...
      final_repo = Enum.reduce(commits_data, repo, fn {message, content}, acc_repo ->
    ...
    test "filters history by author", %{repo: repo} do
      ...
      repo_with_changes = %{repo | working_directory: working_changes}
      {:ok, repo_with_staged} = VersionControl.stage_changes(repo_with_changes, :all)
      {:ok, repo_with_commit} = VersionControl.commit(repo_with_staged, "Different author", "other_user")
      
      test_user_history = VersionControl.get_commit_history(repo_with_commit, %{author: "test_user"})
      other_user_history = VersionControl.get_commit_history(repo_with_commit, %{author: "other_user"})
  ...
  describe "get_diff/3" do
    setup do
      ...
      repo_with_changes = %{repo | working_directory: working_changes}
      {:ok, repo_with_staged} = VersionControl.stage_changes(repo_with_changes, :all)
      {:ok, repo_with_first} = VersionControl.commit(repo_with_staged, "First commit", "test_user")
      ...
      {:ok, repo_with_second} = VersionControl.commit(repo_with_staged2, "Second commit", "test_user")
