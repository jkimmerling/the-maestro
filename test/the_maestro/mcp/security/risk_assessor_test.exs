defmodule TheMaestro.MCP.Security.RiskAssessorTest do
  ...
  describe "classify_risk_level_by_factors/1" do
    test "classifies based on risk factors" do
      assert RiskAssessor.classify_risk_level_by_factors([:safe_path, :read_only_operation]) ==
               :low

      assert RiskAssessor.classify_risk_level_by_factors([:network_access, :external_service]) ==
               :medium

      assert RiskAssessor.classify_risk_level_by_factors([:sensitive_path, :system_access]) ==
               :high

      assert RiskAssessor.classify_risk_level_by_factors([
               :destructive_command,
               :system_modification
             ]) == :critical
    end
    ...
    test "detects dangerous commands" do
      ...
      for command <- dangerous_commands do
        ...
        assert assessment.risk_level in [:high, :critical],
               "Failed to assess risk for: #{command}"
      end
    end
