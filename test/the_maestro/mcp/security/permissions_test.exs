defmodule TheMaestro.MCP.Security.PermissionsTest do
  ...
  describe "check_network_access/3" do
    ...
    test "allows access to wildcard patterns", %{permissions: permissions} do
      result =
        Permissions.check_network_access(permissions, "http://localhost:3000/api", :outbound)
    ...
    test "blocks access to blacklisted domains", %{permissions: permissions} do
      result =
        Permissions.check_network_access(permissions, "https://malicious.com/api", :outbound)
    ...
    test "blocks access to blocked wildcard domains", %{permissions: permissions} do
      result =
        Permissions.check_network_access(permissions, "https://evil.dangerous.net", :outbound)
  ...
  describe "merge_permissions/2" do
    ...
    test "merges resource limits" do
      ...
      # Overridden
      assert merged.resources.max_cpu_percent == 80
      # Preserved
      assert merged.resources.max_memory_mb == 256
      # Added
      assert merged.resources.max_file_size_mb == 200
    end
  end
  ...
  describe "validate_permissions/1" do
    ...
    test "reports validation errors for invalid structure" do
      ...
      assert Enum.any?(
               errors,
               &String.contains?(&1, "resources.max_cpu_percent must be an integer")
             )
    end
  end
end
