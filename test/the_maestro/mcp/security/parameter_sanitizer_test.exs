defmodule TheMaestro.MCP.Security.ParameterSanitizerTest do
  ...
  describe "sanitize_parameters/3" do
    ...
    test "respects strict mode option" do
      ...
      result1 =
        ParameterSanitizer.sanitize_parameters(params, "execute_command", strict_mode: false)

      refute result1.blocked

      # Strict mode - should be blocked due to 'rm' command
      result2 =
        ParameterSanitizer.sanitize_parameters(params, "execute_command", strict_mode: true)
  ...
  describe "sanitize_path/2" do
    test "allows safe paths" do
      ...
      assert {:ok, "/home/user/document.pdf"} =
               ParameterSanitizer.sanitize_path("/home/user/document.pdf")
    end
    ...
    test "enforces allowed paths when specified" do
      ...
      assert {:ok, _} =
               ParameterSanitizer.sanitize_path("/tmp/file.txt", allowed_paths: allowed_paths)

      assert {:error, reason} =
               ParameterSanitizer.sanitize_path("/etc/passwd", allowed_paths: allowed_paths)
  ...
  describe "sanitize_command/2" do
    test "allows safe commands" do
      ...
      assert {:ok, "grep pattern file.txt"} =
               ParameterSanitizer.sanitize_command("grep pattern file.txt")
    end
    ...
    test "respects strict mode for suspicious commands" do
      ...
      assert {:error, reason} =
               ParameterSanitizer.sanitize_command("rm file.txt", strict_mode: true)
  ...
  describe "sanitize_url/2" do
    test "allows safe URLs" do
      ...
      assert {:ok, "http://localhost:3000"} =
               ParameterSanitizer.sanitize_url("http://localhost:3000")
    end
    ...
    test "blocks malicious URL protocols" do
      ...
      assert {:error, _} =
               ParameterSanitizer.sanitize_url("data:text/html,<script>alert('xss')</script>")
    ...
    test "respects allowed protocols" do
      ...
      assert {:ok, _} =
               ParameterSanitizer.sanitize_url("ftp://example.com", allowed_protocols: ["ftp"])

      assert {:error, _} =
               ParameterSanitizer.sanitize_url("https://example.com", allowed_protocols: ["ftp"])
    end
  end
  ...
  describe "validate_parameters_safe?/3" do
    ...
    test "returns true when block_on_suspicion is disabled" do
      ...
      assert ParameterSanitizer.validate_parameters_safe?(params, "execute_command",
