defmodule TheMaestroWeb.SuppliedContextItemLiveTest do
  ...
  @create_attrs %{name: "some name", type: :persona, version: 42, metadata: %{}, text: "some text", tags: %{}}
  @update_attrs %{name: "some updated name", type: :system_prompt, version: 43, metadata: %{}, text: "some updated text", tags: %{}}
  ...
  describe "Index" do
    ...
    test "lists all supplied_context_items", %{conn: conn, supplied_context_item: supplied_context_item} do
    ...
    test "updates supplied_context_item in listing", %{conn: conn, supplied_context_item: supplied_context_item} do
      {:ok, index_live, _html} = live(conn, ~p"/supplied_context")

      assert {:ok, form_live, _html} =
               index_live
               |> element("#supplied-context-items-#{supplied_context_item.id} a", "Edit")
               |> render_click()
    ...
    test "deletes supplied_context_item in listing", %{conn: conn, supplied_context_item: supplied_context_item} do
      {:ok, index_live, _html} = live(conn, ~p"/supplied_context")

      assert index_live |> element("#supplied-context-items-#{supplied_context_item.id} a", "Delete") |> render_click()
      refute has_element?(index_live, "#supplied-context-items-#{supplied_context_item.id}")
    end
  end
  ...
  describe "Show" do
    ...
    test "displays supplied_context_item", %{conn: conn, supplied_context_item: supplied_context_item} do
    ...
    test "updates supplied_context_item and returns to show", %{conn: conn, supplied_context_item: supplied_context_item} do
      ...
      assert {:ok, form_live, _} =
               ...
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/supplied_context/#{supplied_context_item}/edit?return_to=show")
