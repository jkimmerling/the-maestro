defmodule TheMaestroWeb.CliAuthControllerTest do
  ...
  describe "POST /api/cli/auth/device" do
    test "generates device code and verification URLs", %{conn: conn} do
      ...
      assert response["interval"] == 5      # 5 seconds
  ...
  describe "POST /api/cli/auth/authorize" do
    setup do
      ...
      user_code = "TEST12"
      expires_at = DateTime.add(DateTime.utc_now(), 900, :second)  # 15 minutes from now
    ...
    test "renders error with expired user code", %{conn: conn} do
      ...
      user_code = "EXPIR1"
      expires_at = DateTime.add(DateTime.utc_now(), -60, :second)  # 1 minute ago
  ...
  describe "GET /api/cli/auth/poll" do
    ...
    test "returns authorization_pending for pending authorization", %{conn: conn, device_code: device_code} do
      conn = get(conn, ~p"/api/cli/auth/poll?device_code=#{device_code}")
      
      assert json_response(conn, 428)  # Precondition Required
    ...
    test "returns access_token when authorization is complete", %{conn: conn, device_code: device_code} do
  ...
  describe "complete_device_authorization/2" do
    ...
    test "completes device authorization with access token", %{conn: conn, device_code: device_code} do
